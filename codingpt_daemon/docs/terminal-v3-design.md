...
## 6. 재부팅 복원 — 터미널 매니페스트 (2026-09-16)

**증상**: Mac 재부팅 → 앱 자동 실행 → 저장된 레이아웃의 터미널 탭이 실체 없이 남아 "[이 워크스페이스에 열린 터미널이 없습니다] / [연결 끊김 — 재연결 중…]" 두 줄이 무한 반복. cmux 는 레이아웃+터미널이 그대로 돌아오는데 우리는 아니었다.

**원인 3개**(전부 실측):
1. **tmux 세션은 OS 재부팅을 못 넘긴다.** 데몬·앱 재시작은 넘어 살아서 "durable 목록" 으로 믿어 왔지만, 재부팅 뒤엔 소켓 자체가 없다(`error connecting to /private/tmp/tmux-501/codingpt`). 기기들의 레이아웃은 tid 를 가리키는데 실체가 사라진 것.
2. **데몬은 attach 에서 터미널을 절대 만들지 않는다**(죽은 pane 의 자동 재접속이 닫은 터미널을 부활시키는 걸 막는 규칙 — 옳다). 그래서 "없다" 고 답하는데, 그걸 **평문**으로 보냈다.
3. **PC 는 평문을 "연결 끊김" 으로 오판했다.** `_attachedWin` 을 연결 전에 낙관적으로 세우고 close 에서 안 지우니 리컨실러는 "attach 생존" 으로 보고 탭을 안 지우고, onclose 는 매번 "연결 끊김" 을 찍고 재접속 → 루프. 앱(폰)은 같은 평문을 **"버전 불일치" 배너**로 오판했다.

**해결**:
- `terminal-manifest.js` — 데몬이 `<stateDir>/terminals.json` 에 살아 있는 터미널(세션명=tid·워크스페이스 루트·마지막 `pane_current_path`·수동 이름)을 기록한다. 생성 즉시 record, `terminal.close` 즉시 forget, 120s 주기 sync(살아 있는 tmux 가 정본). **데몬 기동 시 서버가 없고 기록이 있으면 같은 tid 로 되살린다**(`restoreIfNeeded`, 로컬 리스너 열기 전). 데몬 수명 중 서버가 사라지면(마지막 셸 exit) "전부 닫힘" 으로 기록을 비운다 → 닫은 건 안 살아나고 잃은 건 살아난다. tid 가 같으니 PC·폰·태블릿 레이아웃이 수정 없이 다시 맞물린다. 프로세스·스크롤백은 재부팅으로 이미 사라진 것이라 되살리지 않는다(cmux 도 같다).
- 데몬: v3 클라에는 "터미널 0개" 를 **EXIT 프레임**(`{reason:'no_terminal'}`)으로 말한다(평문은 v1 전용).
- PC: EXIT 뒤의 close 는 "끊김" 이 아니다(`_exitClose`) — 재접속 루프·메시지 없음, `_scheduleReopen` 이 목록에 터미널이 생길 때까지 조용히 대기. "연결 끊김" 은 한 프레임이라도 받았던 채널이 끊겼을 때만 찍는다(`_v3Live`).
- 회귀: `test/terminal-manifest.test.js`(실 tmux — 복원·닫힘 금지·수명 중 비움·즉시 기록), `term-rewire-host.test.js`(EXIT 프레임 계약).
