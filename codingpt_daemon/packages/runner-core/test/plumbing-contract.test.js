...
//  사유: 유닉스 도메인 소켓 실청취(cpt.sock) + sockPath existsSync 단정 — 계약 2 named pipe 재배선 전
//  해당 재배선/정리 후 이 가드를 제거해 win32 커버리지를 복구한다. (darwin/linux 는 무영향)
if (process.platform === 'win32') {
  require('node:test')('plumbing-contract.test.js: win32 스킵 — 유닉스 도메인 소켓 실청취(cpt.sock) + sockPath existsSync 단정', { skip: true }, () => {});
  return;
}
