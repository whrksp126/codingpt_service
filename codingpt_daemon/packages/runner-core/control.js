...
function dispatchRpc(ws, method, params, ok, fail) {
  ...
  // chat.screen — 대화 바인딩과 무관한 화면 상태(상태줄·모드·선택 화면). 짝이 안 지어진 터미널용 폴링.
  if (method === 'chat.screen') {
    const p = params || {};
    Promise.resolve()
      .then(() => cptServer.chatScreen({ cwd: p.cwd, tid: p.tid != null ? p.tid : p.win, agent: p.agent }))
      .then(ok).catch(fail);
    return;
  }
