...
**컨텍스트 규칙(최우선)**: `cpt` 는 **CodingPT 터미널 안에서만** 쓴다 — 이 환경에 `CPT_WS` 가
없으면 여기는 CodingPT 터미널이 아니므로 `cpt` 를 실행하지 마라(데몬도 워크스페이스 밖 조작
요청은 거부한다). 확실치 않으면 `cpt identify --json` 의 `context` 필드로 판정할 수 있다.
