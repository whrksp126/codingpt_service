...
// 바이브코딩 사용자 프로젝트 — objectstore 사용자 가상 워크스페이스 기반.
// 저장 위치: codingpt/execute/workspace/<userId>/projects/<projectId>/
//   (s3Service 가 codingpt/execute/ prefix 를 자동으로 붙이므로 여기선 workspace/.. 부터 다룬다)
// 각 프로젝트 폴더 안에 메타데이터 project.json + 실제 코드 파일들이 들어간다.
// 프로젝트 "등록" = 해당 경로에 폴더(=project.json 객체) 생성, "목록" = 폴더 나열.
// DB 없이 objectstore 만으로 관리한다(단일/소수 사용자 전제, 메타는 작은 json).

// 경로 주입 방지 — userId/projectId 모두 안전 문자만 허용
const safeUid = (userId) => (userId == null ? '' : String(userId).replace(/[^A-Za-z0-9_-]/g, ''));
const PROJECT_ID_RE = /^[A-Za-z0-9_-]+$/;

const projectsBase = (uid) => `workspace/${uid}/projects`;
const projectDir = (uid, id) => `workspace/${uid}/projects/${id}`;
const metaKey = (uid, id) => `${projectDir(uid, id)}/project.json`;
...
// 짧고 충돌 없는 프로젝트 id (경로 안전 문자만). 이름은 메타에 별도 저장(한글 허용).
function genId() {
...
async function writeMeta(uid, id, meta) {
  ...
  if (!res.success) {
    const e = new Error(res.message || '프로젝트 정보를 저장할 수 없습니다.');
    e.statusCode = 500;
    throw e;
  }
  return meta;
...
/**
 * 사용자 프로젝트 목록 — projects/ 하위 각 폴더의 project.json 을 읽어 메타 배열로.
 * @param {string|number} userId
 * @returns {Promise<Array>} 최신 수정순 정렬
 */
async function listProjects(userId) {
  const uid = requireUid(userId);
  const listed = await s3Service.listFiles(projectsBase(uid), true);
  // 폴더가 아예 없으면(첫 사용자) 빈 목록
  if (!listed.success) {
    if (listed.error === 'NoSuchBucket' || listed.error === 'AccessDenied') {
      const e = new Error(listed.message || '프로젝트 목록을 불러올 수 없습니다.');
      e.statusCode = 500;
      throw e;
    }
    return [];
  ...
  // 각 프로젝트 폴더의 project.json 만 추림 → id 추출
  const ids = [];
...
/**
 * 프로젝트 생성 — 폴더(project.json) 생성. 실제 코드 파일은 이후 에이전트/저장으로 채워진다.
 * @param {string|number} userId
 * @param {{name?:string, description?:string, stack?:string[], thumb?:string}} input
 */
async function createProject(userId, input = {}) {
  const uid = requireUid(userId);
  const name = typeof input.name === 'string' && input.name.trim() ? input.name.trim() : '새 프로젝트';
  const id = genId();
...
async function duplicateProject(userId, id) {
  ...
  const cp = await s3Service.copy(`${projectDir(uid, id)}/`, `${projectDir(uid, newId)}/`);
  if (!cp.success) {
    const e = new Error(cp.message || '프로젝트 복제에 실패했습니다.');
    e.statusCode = 500;
    throw e;
  }
  const now = new Date().toISOString();
...
/** 프로젝트 삭제 — 폴더 내 모든 파일 제거 */
async function deleteProject(userId, id) {
  const uid = requireUid(userId);
  assertProjectId(id);
  const listed = await s3Service.listFiles(projectDir(uid, id), true);
