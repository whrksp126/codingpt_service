...
**ToS 하드 경계 (절대 규칙)** — *2026-07-09 원격 조작 서비스 피벗(M1)으로 갱신*:
비용 0은 "AI가 **사용자 소유 머신에서 사용자 자신의 claude·구독으로** 실행되고, 우리 인프라는
그 결과 바이트만 릴레이한다"는 원칙에서 성립한다. 경계의 핵심은 **실행 위치·자격증명 소유**이지
"헤드리스냐"가 아니다.
- ✅ 허용: PTY 바이트 미러링. **그리고 데몬이 사용자 자신의 로컬 `claude` CLI를 구조화 모드
  (`claude -p --output-format stream-json`)로 spawn하는 것** — 사용자가 터미널에 직접 타이핑하는 것과
  동일하게 **사용자 PC에서·사용자 구독으로** 돈다. 자격증명은 그 PC에만 있고 API 요청은 PC→Anthropic 직결.
- ❌ 금지: 우리 서버/우리 키로 claude·Agent SDK를 구동하거나, 사용자 자격증명(구독 토큰/OAuth)을
  **우리 인프라로 라우팅**하는 모든 형태 (2026-01 Anthropic ToS 금지·서버차단).
- 데몬 코드에는 AI 자격증명(Keychain·`~/.claude` OAuth 등)을 **읽거나 옮기는 코드가 존재해서는 안 된다.**
  (에이전트 세션 재개용 `~/.claude/projects/*.jsonl` **대화 로그** 읽기는 자격증명이 아니므로 허용.)
- GA 전 Anthropic에 BYO spawn 적법성 서면 확인 권고(비차단, dev/local 진행).
