...
- **비용 0 원칙**: 로컬(PC) 경로의 AI는 *사용자가 자기 claude를 자기 구독으로 직접* 실행. 우리 인프라로 사용자 자격증명/구독이 흐르지 않는다.
- **ToS 경계(절대 규칙)**: 데몬은 AI 자격증명(Keychain·`~/.claude`·OAuth)을 **읽지도 다루지도 않는다.** claude 헤드리스 구동(`claude -p`) 금지. 데몬이 하는 건 터미널/파일/포트 **바이트 릴레이**가 전부.
