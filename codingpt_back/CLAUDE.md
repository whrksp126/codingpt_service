...
- 환경별 설정: `config/database.js` — local/development는 SSL 없음, staging/production은 SSL 필요
- **스키마 변경은 반드시 마이그레이션으로** — `sequelize.sync()` 금지. 절차/명령어는 `.claude/rules/db-migration.md` 참조
- 컨테이너 시작 시 `docker-entrypoint.sh`가 `npx sequelize-cli db:migrate` 자동 실행 → 신규 마이그레이션이 자동 적용됨
- 베이스라인: `codingpt_service/db/backups/full_YYYYMMDD.sql` (gitignored, 새 환경 세팅 시 복원 후 `scripts/db-stamp-baseline.sh` 1회 실행)
...
- `ttsStorageService.js` — ObjectStore 저장/삭제
...
| MinIO ObjectStore | 파일 저장소 (objectstore.ghmate.com), env: `OBJECTSTORE_*` |
| ElevenLabs | TTS 오디오 생성 |
| Docker (Dockerode) | 샌드박스 코드 실행 |
| PostgreSQL | local/dev: Docker 컨테이너, staging/prod: 외부 서버 |
